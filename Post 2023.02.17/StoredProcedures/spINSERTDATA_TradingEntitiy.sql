...
CREATE PROC spINSERTDATA_TradingEntitiy
(
...
--------------------------------------------------------
	...
	IF EXISTS (SELECT [Id] FROM [dbo].[TradingSector] WHERE [Name] = @returned_sector_name)
		BEGIN
			SET @SectorId = (SELECT TOP 1 [Id] FROM [dbo].[TradingSector] WHERE [Name] = @returned_sector_name);
			PRINT '    TRADING SECTOR SET';
		END

	ELSE
		BEGIN
			SET @SectorId = (SELECT TOP 1 [Id] FROM [dbo].[TradingSector] WHERE [Name] = 'Unspecified')
			PRINT '    TRADING SECTOR SET TO: UNSPECIFIED';
		END
